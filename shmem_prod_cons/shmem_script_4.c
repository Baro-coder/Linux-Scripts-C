...
int main()
{
    ...
    sem_init(&te->s_cons_1, 1, 0);
    sem_init(&te->s_cons_2, 1, 0);
    sem_init(&te->s_cons_3, 1, 0);
    ...
    else if(PP1 != 0 && PP2 != 0 && PP3 != 0)
    {
        ...
        while(1)
        {
            ...
            if(i <= 4) sem_post(&te->s_cons_1);
            else if(i > 4 && i <= 6) sem_post(&te->s_cons_2);
            else if(i > 6) sem_post(&te->s_cons_3);
    ...
    sem_close(&te->s_cons_2);
    sem_close(&te->s_cons_3);
...
void doConsumer(int cID)
{
    ...
    if(mem == -1)
    {
        fprintf(stderr, "Shared memory does not exists!\n");
        exit(1);
    }
    ...
    while(1)
    {
        if(cID == 1) sem_wait(&te->s_cons_1);
        else if(cID == 2) sem_wait(&te->s_cons_2);
        else if(cID == 3) sem_wait(&te->s_cons_3);
